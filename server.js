...
app.get('/entries', (request, response) => {
  const entries = db.get('entries').sortBy('moduleCode').value();
...
app.get('/entries/:moduleCode', (request, response) => {
  ...
  const entry = db.get('entries').find({ moduleCode: moduleCode }).value();
...
bot.start(async ctx => {
  ...
  if (!ctx.chat.title.match('^[A-Z0-9]+ Discussion$'))
    return ctx.reply('Your group name is invalid. Ensure that it is in the form of "CS1231 Discussion".');
  
  if (ctx.chat.type !== 'supergroup')
    return ctx.reply('Your group is not a supergroup. Please convert it into one.');
  
  const moduleCode = ctx.chat.title.split(' ')[0];
  const chatIdExist = db.get('entries').find({ chatId: ctx.chat.id }).size() > 0;
  const moduleCodeExist = db.get('entries').find({ moduleCode: moduleCode }).size() > 0;
  
  if (entryExist)
    return ctx.reply('This chat has already been registered');
  
  ctx.exportChatInviteLink().then(chatUrl => {
    db.get('entries')
